...
function parsePlaylists() {
  return parseCsvArg(getArg("--playlists") || process.env.SPOTIFY_PLAYLIST_IDS);
}

function parseTerms() {
  const configured = parseCsvArg(getArg("--terms") || process.env.SPOTIFY_SEARCH_TERMS);
  return configured.length > 0 ? configured : DEFAULT_TERMS;
}
...
async function searchArtists(token: string, term: string) {
  const results: SearchArtistItem[] = [];
...
async function main() {
  ...
  const target = Math.max(50, Number(getArg("--target") || process.env.SPOTIFY_DISCOVERY_TARGET || 220));
  const playlistIds = parsePlaylists();
