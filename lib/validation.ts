...
export const artistUpdateSchema = z.object({
  ...
  bio: z.string().trim().min(20).max(1200),
  aiSummary: z.string().trim().max(220).optional().or(z.literal("")),
  spotifyFollowers: z.coerce.number().int().nonnegative().optional(),
  topTrackUrl: optionalUrl,
  topTrackName: z.string().trim().max(180).optional().or(z.literal("")),
  latestReleaseName: z.string().trim().max(180).optional().or(z.literal("")),
  latestReleaseDate: z.string().trim().optional().or(z.literal("")),
