      ...
      <section className="mx-auto w-full max-w-5xl space-y-6 px-4 py-8 md:px-6">
        <div className="space-y-3 rounded-2xl bg-slate-900 p-6 text-white">
          ...
          <p className="text-sm text-slate-200">
            {new Date(dropEvent.date).toLocaleDateString("en-MY", {
              weekday: "long",
              year: "numeric",
              month: "long",
              day: "numeric"
        ...
        <section className="space-y-4">
          ...
          {dropEvent.artists.length === 0 ? <p className="text-slate-600">No approved artists assigned yet.</p> : null}
            ...
            {dropEvent.artists.map((artist) => (
              <article key={artist.id} className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  <Link href={`/artists/${artist.slug}`} className="rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white">
                    View profile
                  </Link>
                  {artist.spotifyUrl ? (
